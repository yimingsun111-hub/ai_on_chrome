...
import { buildSnapshot, performAction, performKey, locateMarkedElement, readViewport, locateFindReplace, fillFindReplace, clickMarked, readDialogText, locateDocsCanvas, showGlow, hideGlow } from "./page.js";
...
// 注意：只认"页面观察"消息（text 以"当前页面："开头）——用户附件里的图片不能被裁掉。
function pruneForSend(messages) {
  ...
  return messages.map((m, i) => {
    if (isObsMsg(m) && i !== n - 1) return { role: m.role, content: "（较早的页面状态已省略以节省额度）" };
    return m;
  });
}
 ...
 */
export async function runTask(task, log, isStopped, history = [], attachments = []) {
    ...
    const followNewTab = async () => {
      ...
      for (const id of ids) {
        try {
          ...
          if (item.windowId === taskWindowId && !isRestricted(item.url || "")) candidates.push(item);
        } catch (_) {}
      }
      const target =
